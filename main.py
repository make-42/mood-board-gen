...
inputFolder = "../Koe no Katachi Filtered/"
...
for file in files:
    print("["+str(round(time.time()-processStartTime))+"] "+str(iterationCount)+"/"+str(len(files))+" "+file,end=" ")
    currentImage = Image.open(inputFolder+file)
    ratios.append(float(currentImage.size[0])/float(currentImage.size[1]))
    ...
    domColor.append(rgbDomColor)
    hsvDomColorList.append(hsvDomColor)
    hDomColorList.append(hsvDomColor[0])
    paletteList.append(color_thief.get_palette(color_count=paletteCount))
    iterationCount+=1
    print("Done.")
print("Done.")
